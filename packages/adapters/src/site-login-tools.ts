...
export function redactVaultToolArgs(
  ...
  if (name !== "vault_put" && !("password" in args)) return args;
...
export async function getSiteLoginFromTool(
  ...
  if (rows.length === 0) {
    return { error: "No vault login for that site." };
  }
  if (rows.length > 1 && !username) {
    return {
      error: "Several accounts match that site. Pass username.",
