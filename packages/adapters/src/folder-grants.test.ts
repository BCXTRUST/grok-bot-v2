...
import { addFolderGrant, loadAllFolderGrants, loadFolderGrants } from "./folder-grants.js";
...
describe("folder grants", () => {
  it("persists per-user grants and unions them for the desktop executor", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "rakazo-grants-"));
    dirs.push(dataDir);
