...
describe("emulators", () => {
  it("writes destination state that an inspector can read", async () => {
    ...
    for await (const event of dest.execute(
      { tool: "destination.write", args: { collection: "notes", title: "hello", body: "world" }, executionId: "e1" },
      ctx,
    )) {
  ...
  it("records MCP writes independently of the model", async () => {
    ...
    await fetch(`http://127.0.0.1:${port}/`, {
      ...
      body: JSON.stringify({ method: "tools/call", params: { name: "notes.write", arguments: { path: "a.md", text: "ok" } } }),
    });
